...
import { Box, Typography, Button, Slider, IconButton, CircularProgress, keyframes, styled } from '@mui/material';
import { PlayArrow, Pause, Replay } from '@mui/icons-material';

// Defina a animação de "pulse" (exemplo)
const pulseAnimation = keyframes`
    0% { transform: scale(1); }
    50% { transform: scale(1.05); } /* Ligeiramente maior no meio */
    100% { transform: scale(1); }
`;

// Crie um StyledCircularProgress para aplicar a animação
const AnimatedCircularProgress = styled(CircularProgress)`
    &.MuiCircularProgress-root.animate { /* Classe 'animate' para ativar a animação */
        animation: ${pulseAnimation} 1.5s ease-in-out; /* Duração e easing da animação */
    }
`;
...
const PomodoroTimer = ({ onTimerComplete }) => {
    ...
    const [animateProgress, setAnimateProgress] = useState(false); // Novo estado para animação
    ...
    const handleStartPause = () => {
        ...
        setAnimateProgress(true); // Inicia a animação ao iniciar/pausar
        setTimeout(() => setAnimateProgress(false), 1500); // Desativa a animação após 1.5s (mesma duração do keyframe)
    };
    ...
    return (
        <Box sx={{ textAlign: 'center' }}>
            ...
            <Box sx={{ position: 'relative', display: 'inline-flex', mb: 2 }}>
                <AnimatedCircularProgress // Use o AnimatedCircularProgress aqui
                    variant="determinate"
                    value={progress}
                    size={120}
                    thickness={2}
                    ...
                    className={animateProgress ? 'animate' : ''} // Aplica a classe 'animate' condicionalmente
                />
